...
export const getEvents = async (context: GetEventsContext) => {
  try {
    const eventsEncontrados = await EventosCollection.find({
      fecha: { $gte: new Date() },
    })
      .sort({ fecha: 1, inicio: 1 })
      .toArray();

    context.response.body = eventsEncontrados;
    ...
    if (context.params?.id) {
      ...
      if (!eventoEncontrado) {
        context.response.body = {
          msg: "No se ha encontrado ningun evento con ese id",
        };
      ...
      context.response.body = {
        msg: "No se ha introducido el parametro id",
      };
